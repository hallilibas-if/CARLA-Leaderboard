...
class violateSpeedLimitTest(Criterion):
    ...
    def __init__(self, actor, max_velocity_threshhold=6,optional=False, name="CheckDiffVelocity"):
        """
        Setup actor and maximum allowed velovity
        """
        super(violateSpeedLimitTest, self).__init__(name, actor, max_velocity_threshhold, None, optional)
        self._actor= actor
        self.max_velocity_threshhold = max_velocity_threshhold
        self.speedLimit= 0
    ...
    def update(self):
        ...
        #Note speed is given in m/s
        self.current_speed = CarlaDataProvider.get_velocity(self.actor)
        self.speedLimit = self._actor.get_speed_limit()
        
        self.actual_value = abs(self.speedLimit - self.current_speed)

        if self.actual_value > 1:
            self.test_status = "FAILURE"
